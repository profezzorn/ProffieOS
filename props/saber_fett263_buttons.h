...
Standard Controls While Blade is OFF
  ...
  Play Track = Long Click PWR
  NEW! Track Player = Hold PWR
  ...
  NEW! Quote Player* = Hold PWR
    *not available is Choreography is enabled
    *sequentially play quote.wav (if missing will play force.wav sound, no effects)
...
#endif
    if (announce_mode_ && !wav_player->isPlaying()) {
      if (rehearse_) {
        if (SaberBase::IsOn()) {
          rehearse_ = false;
          Off();
...
// Fett263 Track Player
  ...
  void DetectPush() {
    ...
    if (fabs(mss.x) < 3.0 &&
        ...
        fabs(fusor.gyro().x) < 5) {
      if (millis() - clash_impact_millis_ > saved_gesture_control.forcepushlen) {
        clash_type_ = CHECK_PUSH;
...
#else
            if (menu_sub_pos_ <= 1) menu_sub_pos_ = 4;  
            menu_sub_pos_ = menu_sub_pos_ - 1;
            switch (menu_sub_pos_) {
              case 1:
                // Color Options
...
#ifndef FETT263_SAVE_CHOREOGRAPHY

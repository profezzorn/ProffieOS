...
Includes 1, or 2 and 3 button controls.
Incorporates an intuitive control scheme so button actions are consistant
whether blade is on or off.
Best if used with ProffieOS_Voicepack spoken menu prompts. (Free)                       // MAKE update comments PR
Download your choice of language and variation here:
http://fredrik.hubbe.net/lightsaber/sound/
Also a large variety of FREE in-universe character Voicepacks available here:       // MAKE update comments PR
https://crucible.hubbe.net/t/additional-voicepacks/4227
If you'd care to make a donation for my time, a coffee would be nice!
...
- Dual blade independent ignition and retraction. (Such as for a staff saber)
...
EFFECT_USER2            - For blade effects with sounds that might work better without hum,
                          ...
                          blade effect too by using TransitionEffectL<......,EFFECT_USER2>. // MAKE update comments PR

---------------------------------------------------------------------------
Optional #defines for use in user config file:
#define BC_ENABLE_AUTO_SWING_BLAST - Multi-blast initiated by simply swinging // MAKE update comments PR
...
#define MAIN_BLADE              - Set blade # to the appropriate blade from BladeConfig.
#define SECOND_BLADE              Ex: #define MAIN_BLADE 1 #define SECOND_BLADE 2
...
Turn ON blade Muted        - 4x Click and hold POW.
...
Prev Preset                - Double Click and hold POW, release after a second. (Click then long Click)
                             * Not supported in Dual Blade mode. Use Scroll Presets rotary control instead to go backwards.
...
On-Demand Batt Level       - Double Click POW.
                             * NOTE * Requires EFFECT_BATTERY_LEVEL to be in blade style,and uses battery.wav sound effect..  // MAKE update comments PR
...
Blaster Blocks             - Double Click POW.
...
Turn ON Blade              - Short Click POW.
Turn ON without Preon      - Short Click POW (while pointing up), or use Gesture Ignition.
                             * Uses fastout.wav if available. If using Dual Blades, this will turn on all blades.
Turn ON blade Muted        - 4x Click and hold POW.
Scroll Presets             - Click and Hold POW to toggle this mode on/off. Rotate hilt to cycle next and previous presets.
                             * TWIST_ON not available in this mode. Click POW to ignite with chosen preset.
...
|   **** Pointing UP/DOWN specific controls are based on the MAIN blade's orientation except for:
|        - Previous/Next preset fast (changing presets while ON) can be done with either blade pointing up.
|        - Controls in the list below can use either blade pointing up.
|
|  Changes to 1 button controls:
|     - No On-Demand (Non-spoken) battery level. (Double Click when OFF)
...
|     - Turn Main Blade ON/OFF      - Double Click POW (Short Click POW still turns on Main blade as well)
|     - Turn Second Blade ON/OFF    - Double Click and hold POW, release after a second (while NOT pointing up).
|                                    - or - 
|                                     If the hilt has an AUX button,
|                                     Double Click and Hold AUX, release after a second.
...
|     - Turn ON Dual Blades         - Double Click and Hold POW. (Turns on all blades)
...
|     - Turn ON without preon       - Do any of the above ignitions while pointing the target blade UP. (Either one if igniting both)
...
#if defined(MAIN_BLADE) && !defined(SECOND_BLADE) || !defined(MAIN_BLADE) && defined(SECOND_BLADE)
#error "Dual Blades use requires both #define MAIN_BLADE and #define SECOND_BLADE"
#endif
...
#endif
#ifdef DUAL_BLADES
#define MAIN_BLADE_BIT (1<<MAIN_BLADE)
#define SECOND_BLADE_BIT (1<<SECOND_BLADE)
#endif
...
EFFECT(mute);      // Plays before mute ignition to avoid confusion. // BC personal
...
BLADE_ID_CLASS_INTERNAL blade_id;
  void Loop() override {
    ...
    if (mute_timer_trigger_ && DetectTimer(mute_timer_trigger_, last_timer_time_, mute_sound_length_)) {
      DoMute();
      mute_timer_trigger_ = false; // Reset mute timer flag
    }
    if (scroll_presets_timer_trigger_ && DetectTimer(scroll_presets_timer_trigger_, last_timer_time_, beep_duration_ * 1000)) {
        if (scroll_presets_) {
            SaberBase::DoEffect(EFFECT_NEWFONT, 0);
        }
        scroll_presets_timer_trigger_ = false; // Reset the timer flag
    }
  }  // Loop()
bool DetectTimer(bool& trigger_flag, uint32_t& last_timer_time, float duration) {
    if (trigger_flag && (millis() - last_timer_time > duration)) {
        trigger_flag = false; // Reset the trigger flag
...
// Volume Menu
...
#ifdef DUAL_BLADES
  void ControlBladeState(bool& this_blade_on_, bool other_blade_on_, bool& this_blade_first_, bool& other_blade_first_, uint16_t BLADE_BIT, bool is_main_blade_) {
    ...
    if (!mode_volume_) {
      if (!this_blade_on_) {
        if (other_blade_on_) {
          SaberBase::DoOn(EffectLocation(0, BLADE_BIT));
        } else {
          this_blade_first_ = true;
          if (is_pointing_up()) {
            FastOn(EffectLocation(0, is_main_blade_ ? ~SECOND_BLADE_BIT : ~MAIN_BLADE_BIT));
          } else {
            if (CommonIgnition()) {
              On(EffectLocation(0, is_main_blade_ ? ~SECOND_BLADE_BIT : ~MAIN_BLADE_BIT));
              on_pending_ = true;
            }
          }
        }
        ...
        SaberBase::DoOff(SaberBase::OFF_NORMAL, EffectLocation(0, BLADE_BIT));
        PVLOG_NORMAL << "** " << curblade << " Blade Deactivated\n";
      } else if (!SaberBase::Lockup()) {
        if (other_blade_first_) {
          Off(off_type, EffectLocation(0, is_main_blade_ ? ~SECOND_BLADE_BIT : ~MAIN_BLADE_BIT));
        } else {
          OffCheckBypassPostoff();  // should turn off all blades that are on...but it's not?
  ...
  void TurnMainBladeOnOff() {
    ControlBladeState(main_blade_on_, second_blade_on_, main_blade_first_, second_blade_first_, MAIN_BLADE_BIT, true);
  }
  void TurnSecondBladeOnOff() {
   ControlBladeState(second_blade_on_, main_blade_on_, second_blade_first_, main_blade_on_, SECOND_BLADE_BIT, false);
  }
#endif //  DUAL_BLADES
  bool is_pointing_up() {
...
// Turn Saber ON Muted
    case EVENTID(BUTTON_POWER, EVENT_FOURTH_HELD, MODE_OFF):
      ...
      if (SFX_mute) {
        ...
        mute_sound_length_ = GetSoundLengthNow(&SFX_mute);
        last_timer_time_ = millis();
        mute_timer_trigger_ = true;
      }
    }
    return true;
#ifdef DUAL_BLADES
    ...
    case EVENTID(BUTTON_POWER, EVENT_SECOND_HELD_MEDIUM, MODE_ON):
        if (!(main_blade_on_ && second_blade_on_)) {  // BOTH ARE NOT ON Turn the other blade on.
          if (main_blade_on_ && !second_blade_on_) {
            TurnSecondBladeOnOff();
            PVLOG_NORMAL << "** Second Blade Activated\n";
...
// Toggle Scroll Presets
    ...
    if (scroll_presets_) {
      ...
      beep_duration_ = 0.2;
      last_timer_time_ = millis();
      scroll_presets_timer_trigger_ = true;
    } else {
...
// Lightning Block
#ifdef DUAL_BLADES
  case EVENTID(BUTTON_POWER, EVENT_FIRST_HELD_MEDIUM, MODE_ON): // Dual blades case
#else
    case EVENTID(BUTTON_POWER, EVENT_SECOND_HELD_MEDIUM, MODE_ON):
#endif
...
private:
  float beep_duration_ = 0.0;
  float current_menu_angle_ = 0.0;
  float mute_sound_length_ = 0.0;
  bool mute_timer_trigger_ = false;
  bool scroll_presets_timer_trigger_ = false;

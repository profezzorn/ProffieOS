/* Created by OlivierFlying747-8 based on detonator.h by Fredrik Hubinette
  ...
  Copyright (c) 2025 OlivierFlying747-8
...
You can arm/disarm it or arm and make it detonate (via clash for immediate detonation or via timer for delayed detonation).
...
This detonator version uses pow & aux buttons (unlike detonator.h wich uses pow & aux2).
...
============= Optional detonator define that you can add to your config. ========================================================
#define BUTTON_POWER_IS_LATCH                  // if your pow button is laching and has been defined as such in your config.
#define DETONATOR_TIMER_DURATION 6.0f          // default is 6 seconds during "loud timer" delayed detonation.
#define DETONATOR_STEALTH_TIMER_DURATION 12.0f // default is 12 seconds during "quiet timer" delayed detonation.
...
AUX Button:
  ...
  - Press and hold (ON or OFF) : Enter/Exit Volume Menu
  - Short click (while in Volume Menu):
      - Pointing UP   → Volume Up
      - Pointing DOWN → Volume Down
      - Level         → No action
  - Short click while OFF (not in Volume Menu): Next Preset

CLASH (while ON and armed):
  - Instantly trigger boom (interrupts any countdown), resets everything
...
#ifndef PROPS_DETONATOR_OLI_BUTTONS_H // v11 - v8 works perfectly with or without multi_prop. DO NOT TOUCH IT !!!
...
Props in alphabetical order, Sabers First, Blaster(s) Second.
*/
...
public:
  ...
  bool Event2(enum BUTTON button, EVENT event, uint32_t modifiers) override {
    switch (EVENTID(button, event, modifiers)) {
      // ON
#ifdef BUTTON_POWER_IS_LATCH
      case EVENTID(BUTTON_POWER, EVENT_LATCH_ON, MODE_OFF):
#else
      case EVENTID(BUTTON_POWER, EVENT_FIRST_SAVED_CLICK_SHORT, MODE_OFF):
#endif
        if (timer_active_) {
          return true; // prevents power-on during countdown!
        } else {
          armed_ = false;
          SetPower(true);
          On();
...
#endif
        if (timer_active_) {
          return true; // prevents power-off during countdown!
        } else {
          armed_ = false;
          SetPower(false);
          Off();
      ...
      // ARMED or START TIMER (6 seconds) or Volume Up/Down (while ON & in Volume menu)
      ...
      case EVENTID(BUTTON_AUX, EVENT_FIRST_HELD_MEDIUM, MODE_ON):
        if (timer_active_) {
          return true; // prevents interrupting countdown!
        } else {
          if (!armed_) {
            armed_ = true;
          }
      ...
      // Next Preset or Volume Up/Down (while OFF & in Volume menu)
  ...
  void SB_Effect(EffectType effect, EffectLocation location) override {
    switch (effect) {
      ...
      case EFFECT_VOLUME_LEVEL:                                                              return;
        // Typically used to apply a dimming effect on blade to conserve battery. Here it will be used for "stealth mode".
      ...
      case EFFECT_BATTERY_LEVEL:    hybrid_font.PlayPolyphonic(&SFX_battery);
                                    PVLOG_VERBOSE << "*** May the Force be with you. ***\n"; return;
    }
  }  // SB_Effect
...
// At the moment, none of the below are active. Some 'SaberBase::DoEffect(EFFECT_***PICKONE***,0);' need to be added to the code above!

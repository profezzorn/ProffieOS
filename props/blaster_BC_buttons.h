...
Power On / Off            - Click POW. - TODO: this in dual_prop is....weird
...
// Volume Menu
  ...
  void Loop() override {
    ...
    STATE_MACHINE_BEGIN();
    while (true) {
      if (auto_firing_ && millis() - auto_time_ > 250) {
        shots_fired_++;
        auto_time_ = millis();
...
// Spoken Battery Level in volts / show battery meter on LEDs
      ...
      case EVENTID(BUTTON_MODE_SELECT, EVENT_THIRD_SAVED_CLICK_SHORT, MODE_OFF):
        ...
        // TODO: Make battery meter show on OLED for ProffieOSFontImageDuration 
...
// Blade Detect
  ...
  void SB_Effect(EffectType effect, float location) override {
    switch (effect) {
      ...
      case EFFECT_DESTRUCT: hybrid_font.PlayCommon(&SFX_destruct); return; // BC TODO

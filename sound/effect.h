...
class Effect {
  ...
  PO_isDigit(rest[3])) {
    ...
    PVLOG_DEBUG << "Counting " << filename << " as " << name_ << "\n";
    num_files_++;
    if (alt != -1) {
  ...
  void Show() {
    if (files_found()) {
      PVLOG_STATUS.print("Found ");
      PVLOG_STATUS.print(name_);
      PVLOG_STATUS.print(" files: ");
      if (min_file_ <= max_file_) {
        PVLOG_STATUS.print(min_file_);
        PVLOG_STATUS.print("-");
        PVLOG_STATUS.print(max_file_);
        if (digits_) {
          PVLOG_STATUS.print(" using ");
          PVLOG_STATUS.print(digits_);
          PVLOG_STATUS.print(" digits");
        }
      ...
      if (unnumbered_file_found_) {
        PVLOG_STATUS.print("one unnumbered file");
      }
	...
	expected_files *= num_alternatives;
      ...
      switch (file_pattern_) {
        ...
        case FilePattern::SUBDIRS:
          PVLOG_STATUS.print(" in subdirs");
          break;
        case FilePattern::NONREDUNDANT_SUBDIRS:
          PVLOG_STATUS.print(" in efficient subdirs");
	  break;
      }
      if (sub_files_) {
	PVLOG_STATUS.print(" with ");
	PVLOG_STATUS.print(sub_files_);
	PVLOG_STATUS.print(" sub files ");
	expected_files *= sub_files_;
      }
      if (expected_files != (int)num_files_) {
	PVLOG_STATUS << " SOME FILES ARE MISSING! " << expected_files << " != " << num_files_;
      }
      PVLOG_STATUS.print(" in ");
      PVLOG_STATUS.print(directory_);
      PVLOG_STATUS.println("");
    }
  }
  ...
  static void ShowAll() {
    ...
    PVLOG_STATUS.println("Done listing effects.");
  }
...
#define RANDOMIZE(N, LAST) (rand() % (N))  
  ...
  static void ScanOneDirectory(const char* dir) {
    PVLOG_STATUS.print("Scanning sound font: ");
    PVLOG_STATUS.print(dir);
    ...
    if (LSFS::Exists(dir)) {
      Scanner scanner;
      scanner.Scan(dir);
      PVLOG_STATUS.println(" done");
    } else {
      PVLOG_STATUS.println(" NOT FOUND!");
  ...
  static void ScanCurrentDirectory() {
	...
	if (!warned) {
	  warned = true;
	  PVLOG_STATUS.println("");
	  PVLOG_STATUS.println("WARNING: This font seems to be missing some files!!");
